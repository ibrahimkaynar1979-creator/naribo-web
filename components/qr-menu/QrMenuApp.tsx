...
function ProductCard({product,onClick}:{product:QrMenuProduct;onClick:()=>void}){return <button className="qr-product-card" onClick={onClick}><div className="qr-product-image"><img src={product.image} alt={product.name}/></div><div className="qr-product-body"><strong>{product.name}</strong><p>{product.description}</p><div><b>₺{product.price}</b><span>İncele</span></div></div></button>}
function BranchScreen({restaurant}:{restaurant:QrMenuRestaurant}){const b=restaurant.branch;return <div className="qr-page"><AppHeader restaurant={restaurant} title="Şube"/><div className="qr-branch-hero"><h2>{restaurant.name}</h2><div className="qr-rating"><Star size={17} fill="currentColor"/> {restaurant.reviewSummary?.rating??'—'} <span>({restaurant.reviewSummary?.count??0})</span></div></div><div className="qr-list-card"><InfoRow icon={<MapPin size={20}/>} label="Adres" value={`${b.address}, ${b.city}`}/><InfoRow icon={<Phone size={20}/>} label="Telefon" value={b.phone??'—'}/><InfoRow icon={<Info size={20}/>} label="Çalışma Saatleri" value={b.openingHours??'—'}/><InfoRow icon={<Mail size={20}/>} label="E-posta" value={b.email??'—'}/></div></div>}
...
function ContactScreen({restaurant}:{restaurant:QrMenuRestaurant}){return <div className="qr-page qr-contact"><div className="qr-contact-icon"><Send size={30}/></div><h1>Bize Ulaşın</h1><p>{restaurant.name} ile iletişim kurmak için aşağıdaki kanalları kullanabilirsiniz.</p><div className="qr-list-card"><InfoRow icon={<Phone size={20}/>} label="Telefon" value={restaurant.branch.phone??'—'}/><InfoRow icon={<Mail size={20}/>} label="E-posta" value={restaurant.branch.email??'—'}/><InfoRow icon={<MapPin size={20}/>} label="Adres" value={`${restaurant.branch.address}, ${restaurant.branch.city}`}/></div></div>}
...
function BottomNav({tab,onChange}:{tab:Tab;onChange:(t:Tab)=>void}){const items:[Tab,string,React.ReactNode][]=[['home','Ana Sayfa',<Home key="h" size={20}/>],['menu','Menü',<Search key="m" size={20}/>],['branch','Şube',<Building2 key="b" size={20}/>],['reviews','Yorumlar',<MessageCircle key="r" size={20}/>],['contact','İletişim',<Send key="c" size={20}/>]];return <nav className="qr-bottom-nav">{items.map(([id,label,icon])=><button key={id} className={tab===id?'is-active':''} onClick={()=>onChange(id)}>{icon}<span>{label}</span></button>)}</nav>}
function ProductSheet({product,onClose,onBackToMenu}:{product:QrMenuProduct|null;onClose:()=>void;onBackToMenu:()=>void}){return <QrSheet open={!!product} onClose={onClose}>{product&&<div className="qr-detail"><div className="qr-detail-media"><img src={product.image} alt={product.name}/><button onClick={onClose}><X size={25}/></button></div><div className="qr-detail-body"><h2>{product.name}</h2><b className="qr-detail-price">₺{product.price}</b><p>{product.description}</p>{product.allergens?.length?<div className="qr-allergens">{product.allergens.map(a=><span key={a}>{a}</span>)}</div>:null}{product.options?.length?<div className="qr-options"><small>Menü / Porsiyon Seçenekleri</small>{product.options.map((o,i)=><div key={o.id} className={i===0?'is-selected':''}><span>{o.name}</span><b>₺{o.price}</b><i aria-hidden="true"/></div>)}</div>:null}<button className="qr-primary" onClick={onBackToMenu}><ChevronLeft size={18}/> Menüye Dön</button></div></div>}</QrSheet>}
