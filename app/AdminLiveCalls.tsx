...
function ago(v:string){const ms=Date.now()-new Date(v).getTime(),m=Math.max(0,Math.floor(ms/60000));if(m<1)return'Şimdi';if(m<60)return`${m} dk önce`;const h=Math.floor(m/60);return h<24?`${h} sa önce`:`${Math.floor(h/24)} gün önce`}
...
export default function AdminLiveCalls(){
 ...
 useEffect(()=>{if(!slug)return;let dead=false;const syncBadges=(n:number)=>{document.querySelectorAll<HTMLElement>('.qaBell i,.qaSide button em').forEach(el=>{if(n>0){el.textContent=String(n);el.style.display=''}else el.style.display='none'})};const poll=async(first=false)=>{if(busy.current)return;busy.current=true;try{const r=await fetch(`/api/qr-menu/${slug}`,{cache:'no-store'});if(!r.ok)return;const d=await r.json() as ApiData;if(dead)return;const list=(d.calls||[]).filter(c=>c.status==='pending').sort((a,b)=>+new Date(b.createdAt)-+new Date(a.createdAt));setCalls(d.calls||[]);syncBadges(list.length);const ids=new Set(list.map(c=>c.id));if(seen.current===null||first){seen.current=ids;return}const fresh=list.filter(c=>!seen.current!.has(c.id));seen.current=ids;if(fresh.length){const newest=fresh[0];setToast(newest);setTimeout(()=>setToast(null),9000);if(typeof Notification!=='undefined'&&Notification.permission==='granted'){new Notification('Yeni Garson Çağrısı',{body:`Masa ${newest.tableNo}${newest.note?` · ${newest.note}`:''}`,tag:`waiter-${newest.id}`})}}catch{}finally{busy.current=false}};void poll(true);const id=setInterval(()=>void poll(),5000);const onFocus=()=>void poll();window.addEventListener('focus',onFocus);return()=>{dead=true;clearInterval(id);window.removeEventListener('focus',onFocus)}},[slug]);
 const resolve=async(id:string)=>{try{const r=await fetch(`/api/qr-menu/${slug}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'resolveCall',id})});if(!r.ok)throw new Error();setCalls(x=>x.map(c=>c.id===id?{...c,status:'resolved',resolvedAt:new Date().toISOString()}:c));setToast(t=>t?.id===id?null:t)}catch{alert('Çağrı tamamlanamadı. Tekrar deneyin.')}};
 const enable=async()=>{if(typeof Notification==='undefined')return;const p=await Notification.requestPermission();setPermission(p)};
 if(!slug)return null;
 return <>
