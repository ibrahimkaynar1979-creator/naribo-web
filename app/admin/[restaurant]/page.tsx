...
function Editor({restaurant,p,cats,isNew,close,save,del}:{restaurant:Restaurant;p:Product;cats:Category[];isNew:boolean;close:()=>void;save:(p:Product)=>void|Promise<void>;del:(id:string)=>void}){
 const[d,setD]=useState(p);
 const[saveNotice,setSaveNotice]=useState<'added'|'saved'|null>(null);
  ...
  <aside className="qaWorkbenchPreview">
   ...
   <div className="qaLiveMenuPhone">
    <div className="qaLiveMenuTop">
      ...
      <div className="qaLiveMenuLogo">{restaurant.logo?<img src={restaurant.logo} alt={restaurant.name}/>:<strong>{restaurant.name}</strong>}</div>
      <div className="qaLiveLang"><span>TR</span><b>⌄</b></div>
    </div>
    <img className="qaLiveMenuHero" src={d.image||'/makarilla_tabak.png'} alt={d.name||'Ürün'}/>
    <div className="qaLiveMenuBody">
      <div className="qaLiveMenuCategory">{(cats.find(c=>c.id===d.categoryId)?.name||'Kategori').toLocaleUpperCase('tr-TR')}</div>
      <div className="qaLiveMenuTitleRow"><h4>{d.name||'Ürün Adı'}</h4><strong>₺{Number(d.price||0).toLocaleString('tr-TR')}</strong></div>
      <div className="qaLiveMenuRule"/>
      <p>{d.description||'Ürün açıklaması burada görünecek.'}</p>
      {(d.allergens||[]).length>0&&<div className="qaLiveMenuAllergens"><b>ALERJENLER</b><div>{(d.allergens||[]).map(a=><span key={a}>{a}</span>)}</div></div>}
      {d.calories?<div className="qaLiveMenuCalories">{d.calories} kcal</div>:null}
      <div className="qaLiveMenuFootnote"><span>Menü bilgilendirme amaçlıdır · Sipariş alınmamaktadır.</span><span>Powered By <b>paneltakip.com</b></span></div>
    </div>
    <div className="qaLiveMenuBottomNav">
      <div><Wifi size={16}/><span>Wi‑Fi</span></div>
      <div><span className="qaIbanIcon">▭</span><span>IBAN</span></div>
      <div className="active qaWaiter"><span className="qaWaiterCircle"><Bell size={18}/></span><span>Garson Çağır</span></div>
      <div><MessageSquareText size={16}/><span>Geri Bildirim</span></div>
