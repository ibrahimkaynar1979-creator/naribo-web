...
export async function GET(){
  ...
  }catch(error){console.error(error);return NextResponse.json({error:'Menü yüklenemedi'},{status:500});}
}
...
export async function PUT(request:Request){
  try{
    ...
    const incomingIds=new Set(body.products.map(p=>p.id).filter(id=>id&&/^\d+$/.test(id)) as string[]);
    ...
    for(const product of body.products){
      ...
      if(product.id&&/^\d+$/.test(product.id)){
        await sql`UPDATE products SET category_id=${categoryId},name=${product.name.trim()},description=${product.description||''},price=${Number(product.price)||0},image_url=${product.image||''},allergens=${allergens},is_active=${product.isActive},sort_order=${product.sortOrder||0},updated_at=now() WHERE id=${Number(product.id)} AND restaurant_id=${restaurantId}`;
      }else{
        await sql`INSERT INTO products (restaurant_id,category_id,name,description,price,image_url,allergens,is_active,is_featured,sort_order) VALUES (${restaurantId},${categoryId},${product.name.trim()},${product.description||''},${Number(product.price)||0},${product.image||''},${allergens},${product.isActive},false,${product.sortOrder||0})`;
      }
    }
    return NextResponse.json({ok:true});
  }catch(error){console.error(error);return NextResponse.json({error:'Menü kaydedilemedi'},{status:500});}
}
