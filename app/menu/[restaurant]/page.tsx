...
import {Bell,ChevronLeft,Clock3,MapPin,MessageSquareMore,Phone,Search,Star,X} from 'lucide-react';
import './menu.css';
...
export default function CustomerMenu({params}:{params:Promise<{restaurant:string}>}){
 ...
 useEffect(()=>{params.then(p=>setSlug(p.restaurant))},[params]);
 useEffect(()=>{if(!slug)return;let alive=true;(async()=>{setLoading(true);try{const r=await fetch(`/api/menu/${slug}`,{cache:'no-store'});const j=await r.json();if(!r.ok)throw new Error(j.error||'Menü yüklenemedi');if(!alive)return;setData(j);setError('');if(!viewed.current){viewed.current=true;fetch(`/api/menu/${slug}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'menuView'})}).catch(()=>{})}}catch(e){if(alive)setError(e instanceof Error?e.message:'Menü yüklenemedi')}finally{if(alive)setLoading(false)}})();return()=>{alive=false}},[slug]);
 ...
 const openProduct=(p:Product)=>{setSelected(p);setModal('product');fetch(`/api/menu/${slug}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'productView',productId:p.id})}).catch(()=>{})};
 const sendFeedback=async()=>{if(!data||sending)return;setSending(true);try{const r=await fetch(`/api/menu/${slug}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'feedback',rating,comment,customerName})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Gönderilemedi');setModal(null);setComment('');setCustomerName('');setRating(5);alert('Teşekkürler. Geri bildiriminiz alındı.')}catch(e){alert(e instanceof Error?e.message:'Gönderilemedi')}finally{setSending(false)}};
 const sendCall=async()=>{if(!tableNo.trim()||sending)return;setSending(true);try{const r=await fetch(`/api/menu/${slug}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'serviceCall',tableNo, note:callNote})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Çağrı gönderilemedi');setModal(null);setTableNo('');setCallNote('');alert('Garson çağrınız iletildi.')}catch(e){alert(e instanceof Error?e.message:'Çağrı gönderilemedi')}finally{setSending(false)}};
 ...
 return <main className="cm" style={{'--brand':r.themeColor||'#f46861'} as React.CSSProperties}>
  <section className="cmHero" style={r.cover?{backgroundImage:`linear-gradient(180deg,rgba(0,0,0,.18),rgba(0,0,0,.64)),url(${r.cover})`}:{}}>
   <div className="cmHeroTop"><button onClick={()=>history.back()}><ChevronLeft/></button><span>QR MENÜ</span></div>
