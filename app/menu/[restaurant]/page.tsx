...
export default function CustomerMenu({params}:{params:Promise<{restaurant:string}>}){
 ...
 useEffect(()=>{if(!slug)return;let alive=true;(async()=>{setLoading(true);try{const r=await fetch(`/api/menu/${slug}`,{cache:'no-store'});const j=await r.json();if(!r.ok)throw new Error(j.error||'Menü yüklenemedi');if(!alive)return;setData(j);setError('');if(!isPreview&&!viewed.current){viewed.current=true;fetch(`/api/menu/${slug}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'menuView'})}).catch(()=>{})}}catch(e){if(alive)setError(e instanceof Error?e.message:'Menü yüklenemedi')}finally{if(alive)setLoading(false)}})();return()=>{alive=false}},[slug,isPreview]);
 ...
 const openProduct=(p:Product)=>{setSelected(p);setModal('product');if(!isPreview)fetch(`/api/menu/${slug}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'productView',productId:p.id})}).catch(()=>{})};
